...
class LibrispeechDataset(Dataset):
    def __init__(self, data_path, batch_size, max_label_len, bucketing, listener_layer, drop_last=False,
                 training=False):
    ...
    def __getitem__(self, index):
        if not self.bucketing:
            ...
            if self.training:
                ...
                pad_len = len(X[0]) if (len(X[0]) % self.time_scale) == 0 else len(X[0]) + (
                            self.time_scale - len(X[0]) % self.time_scale)
                if self.training:
...
def create_dataloader(data_path, max_label_len, batch_size, shuffle, bucketing, listener_layer, drop_last=False,
                      training=False,
                      **kwargs):
    if not bucketing:
        return DataLoader(
            LibrispeechDataset(data_path, batch_size, max_label_len, bucketing, listener_layer, training=training),
            batch_size=batch_size, shuffle=shuffle, drop_last=drop_last)
    else:
        return DataLoader(
            LibrispeechDataset(data_path, batch_size, max_label_len, bucketing, listener_layer, drop_last=drop_last,

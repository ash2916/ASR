...
- `kernel_size` : Value of convolution kernel size
- `num_channels` : The number of channels in the convolution filter
- `decoder_hidden_state_dim` : The number of features in the decoder hidden state
- `encoder_dim` : Dimension of encoder output vector
- `decoder_output_dim` : Dimension of decoder output vector
- `decoder_dropout_p` : Dropout probability of decoder
...
- `save_checkpoint_n_steps` : Save a checkpoint every N steps.
...
- `sampler` : smart: batching with similar sequence length.else: random batch
- `name` : Trainer name

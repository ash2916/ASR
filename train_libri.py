...
from las.las_model import Listener, Speller
import numpy as np
from torch.autograd import Variable
import torch
import time

conf = yaml.load(open("config/las_libri_config.yaml", 'r'))
...
while global_step < total_steps:
    ...
    tf_rate = tf_rate_upperbound - (tf_rate_upperbound - tf_rate_lowerbound) * min((float(global_step) / tf_decay_step),
                                                                                   1)
